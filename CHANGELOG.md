...
- Refreshed the verified Kokoro archive size and SHA-256 after the upstream release asset was replaced.
...
- Redesigned PWA updates around a waiting service worker: open tabs retain the previous controller and shell-cache generation until reload, while activation removes stale generations only after the replacement can safely take over; first installs still perform the one-time isolation reload.
- Tightened the production CSP so document scripts cannot execute `blob:` modules; worker, media, image, and connection blob paths remain explicitly scoped to their required destinations.
- Added versioned generation provenance manifests for saved clips, resumable jobs, ZIP/project exports, and browser/native M4B metadata. Manifests capture runtime/model, voice, synthesis, cleanup, pronunciation, BGM, encoder, source-hash, cue, and RVC context without exporting source text or article URLs by default; legacy records migrate with replay-drift warnings (521 tests across 90 files).
- Added a reversible text-normalization preview for imported and synthesis-bound text. Rule-grouped diffs cover URLs, citations, acronyms, numbers, page artifacts, PDF reflow, and punctuation pauses; users can toggle rules, apply or undo changes, restore the raw import, and inspect the exact synthesized-text snapshot (524 tests across 90 files).
- Added searchable and sortable clip-library views with voice, engine, and cue filters; storage usage now shows the 200 MB cap and oldest-first eviction context, while per-item audio remains lazy and missing blobs have a clear recovery path (532 tests across 91 files).
- Added a machine-readable capability manifest for app/platform, engine, queue/export, model-license, and runtime-license facts. The renderer registry, diagnostics, model/license tables, release identity, README, and CLAUDE fact blocks now consume or validate that source; `npm run capabilities:update` refreshes test totals and generated documentation (536 tests across 92 files).
- Added a deterministic CycloneDX 1.7 software/model SBOM for runtime packages and every pinned web/native model payload, with repository, revision, SHA-256, license, route, and execution metadata. Pages deploys publish and probe the SBOM, Windows releases attach the validated versioned asset, and diagnostics links to the correct artifact.
- Added a locale-ready UI boundary with a persisted reviewed-English interface setting; engine synthesis-language selectors remain separate, and no unreviewed translations ship.
...
- Hardened the Chatterbox voice lab with an explicit ownership/permission acknowledgement before reference selection, immediate in-memory cleanup when consent is withdrawn, a permissive-license gate for reference-capable models, and local provenance metadata on generated and saved clips without persisting source audio.
...
- Added desktop and mobile PWA install screenshots to the manifest, raised the voice-preview URL cache to an explicit 20-entry LRU cap, and made the runtime license gate fail when a direct production dependency is absent from its table.
- Added a permission-minimal MV3 companion extension: selection and page context actions extract bounded text with temporary active-tab access and open the PWA handoff route; `npm run extension:build` writes a self-contained ZIP.
- Pinned the Pages Kokoro and Piper-plus sync manifests to immutable Hugging Face revisions with exact sizes and SHA-256 digests. Runtime Kokoro fallback, offline prefetch, and cached voice bins now fail closed on mismatched bytes; the Piper-plus browser fallback no longer resolves mutable model revisions.
- Added a v1 engine-adapter SDK and validated local manifest format. Adapter manifests declare immutable model files, runtime/license/safety metadata, hardware needs, queue/export capabilities, and required diagnostics fields; duplicate or malformed adapters fail before registration, without changing the app shell (508 tests across 87 files).
...
- Extracted the public renderer entrypoint, abort-aware generation dispatcher, object-URL registry, and queue/library/generation state hooks; direct dispatcher, URL-lifetime, and hook-state tests now cover success, cancellation, missing-audio, and independent state updates (457 tests across 79 files).
...
- Added service-worker lifecycle coverage for waiting updates, safe old-generation cleanup, and first-install isolation reload behavior (513 tests across 88 files).
- Added a CSP regression contract plus built-preview smoke coverage across the browser engine/control paths (514 tests across 89 files).
...
- Added runtime license-table dependency-drift coverage and smoke validation for shipped PWA screenshot assets (487 tests across 82 files).
